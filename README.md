...
The home page (`/`) now lists every customer/prospect as a card. Click
...
through. All plans are still stored in the same `data/db.json` file, so
this doesn't cost anything extra or need a separate database.

If you already deployed the old single-plan version, this update migrates
your existing data automatically into a plan called after your old title
